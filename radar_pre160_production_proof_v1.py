...
CI run succeeds. Its protected digest excludes the proof file itself, so recording the
proof does not invalidate the audited runtime code. This module never creates proof.
"""
...
def protected_digest(root='.', paths=PROTECTED_PATHS):
    ...
    h = hashlib.sha256()
    missing = []
    for rel in sorted(str(x) for x in paths):
        path = root / rel
    ...
    return {'digest': h.hexdigest() if not missing else None, 'missing': missing, 'files': len(tuple(paths))}
...
def production_proof_status(root='.', proof_path=DEFAULT_PROOF_PATH, paths=PROTECTED_PATHS):
    ...
    if current['missing']:
        return {'status': 'FAILED', 'verified': False, 'reason': 'PROTECTED_FILES_MISSING', 'missing': current['missing'], 'real_trading': False}
    if not path.exists():
        return {
            'status': 'NOT_VERIFIED', 'verified': False, 'reason': 'EXTERNAL_PRODUCTION_PROOF_NOT_RECORDED',
            'protected_digest': current['digest'], 'real_trading': False,
        }
    try:
