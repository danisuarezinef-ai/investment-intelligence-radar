...
def worker_loop():
    ...
    write_status(version='1.1.2',state='INICIANDO',started_at=now(),last_error='',cloud_enabled=True)
    next_market=next_sec=next_science=0
    try:
        while True:
            if not cloud_enabled():
                write_status(state='PAUSADO CLOUD',cloud_enabled=False,current_job=''); time.sleep(2); continue
            t=time.time(); write_status(state='ACTIVO',cloud_enabled=True)
            try:
                if t>=next_market:
                    write_status(state='RECOPILANDO MERCADO',current_job='market')
                    n=collect_market(); print(f'[collector] market added={n}',flush=True); next_market=t+300
                if t>=next_sec:
                    write_status(state='RECOPILANDO SEC',current_job='sec')
                    n=collect_sec(); print(f'[collector] sec added={n}',flush=True); next_sec=t+900
                if t>=next_science:
                    write_status(state='RECOPILANDO CIENCIA',current_job='science')
                    n=collect_science(); print(f'[collector] science added={n}',flush=True); next_science=t+1800
...
def _authorized(handler):
    ...
    return handler.headers.get('Authorization','') == 'Bearer '+CONTROL_TOKEN
...
class _Handler(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        if path=='/toggle':
            ...
            enabled=set_cloud_enabled(not cloud_enabled()); self._send(200,{'ok':True,'cloud_enabled':enabled,'status':_read_status()}); return
        if path=='/collect-now':
            if not _authorized(self): self._send(401,{'ok':False,'error':'unauthorized'}); return
            results={'market':collect_market(),'sec':collect_sec(),'science':collect_science()}; self._send(200,{'ok':True,'results':results,'snapshot':snapshot_payload()}); return
        self._send(404,{'ok':False,'error':'not found'})
...
def main():
    ...
    if port:
        t=threading.Thread(target=worker_loop,name='radar-worker',daemon=True); t.start(); p=int(port)
        print(f'Radar cloud HTTP listening on 0.0.0.0:{p}',flush=True); ThreadingHTTPServer(('0.0.0.0',p),_Handler).serve_forever()
    else: worker_loop()
