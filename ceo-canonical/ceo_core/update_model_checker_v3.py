...
ACTIONS = (
    "stage_a_new", "stage_b_new", "stage_a_old", "stage_b_old", "preflight_ok", "preflight_fail",
    "activate", "health_ok", "health_fail", "power_loss", "lock", "unlock", "tamper_journal",
    "repair_journal", "corrupt_receipt", "repair_receipt", "break_provenance", "repair_provenance",
    "break_parity", "repair_parity", "lose_snapshot", "repair_snapshot", "quarantine_staged", "rollback",
)


def _quarantined(s: UpdateStateV3, c: str) -> bool:
    return (c == "a" and s.quarantine_a) or (c == "b" and s.quarantine_b)


def _evidence_ok(s: UpdateStateV3) -> bool:
    return s.journal_ok and s.receipt_ok and s.provenance_ok and s.parity_ok and s.snapshot_ok


def _errors(s: UpdateStateV3) -> list[str]:
    out: list[str] = []
...
def _step(s: UpdateStateV3, action: str) -> UpdateStateV3:
    d = asdict(s)
    if action.startswith("stage_"):
        _, c, age = action.split("_")
        seq = s.stable_seq + 1 if age == "new" else max(0, s.stable_seq - 1)
        if not s.pending and not s.lock and _evidence_ok(s) and c != s.pointer and not _quarantined(s, c) and seq >= s.stable_seq:
            d.update(staged=c, staged_seq=seq, preflight="")
    elif action == "preflight_ok" and s.staged and not s.pending and _evidence_ok(s) and not _quarantined(s, s.staged):
        d["preflight"] = s.staged
    elif action == "preflight_fail" and s.staged and not s.pending:
        d.update(staged="", staged_seq=0, preflight="")
    elif action == "activate" and s.staged and s.preflight == s.staged and not s.pending and not s.lock and _evidence_ok(s) and s.staged_seq >= s.stable_seq:
    ...
    elif action == "break_provenance" and not s.pending: d["provenance_ok"] = False
    elif action == "repair_provenance" and not s.pending: d["provenance_ok"] = True
    elif action == "break_parity" and not s.pending: d["parity_ok"] = False
    elif action == "repair_parity" and not s.pending: d["parity_ok"] = True
    elif action == "lose_snapshot" and not s.pending: d["snapshot_ok"] = False
    elif action == "repair_snapshot" and not s.pending: d["snapshot_ok"] = True
    elif action == "quarantine_staged" and s.staged and not s.pending:
        d["quarantine_" + s.staged] = True
        d.update(staged="", staged_seq=0, preflight="")
    return UpdateStateV3(**d)


def bounded_model_check_v3(*, max_depth: int = 17, max_states: int = 500_000) -> dict[str, Any]:
    start = UpdateStateV3()
    q = deque([(start, 0)])
    seen = {start}
    transitions = 0
