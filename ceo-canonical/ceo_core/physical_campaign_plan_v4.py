...
LOCAL_PRECONDITIONS = (
    ...
    "android_source_harness",
    "cross_platform_parity",
    "cross_platform_provenance",
    "transaction_ledger",
    "model_checker_v3",
    "convergence_gate_v2",
)
...
def build_one_shot_windows_campaign_v4(local_preconditions: dict[str, bool]) -> dict[str, Any]:
    ...
    return {
        ...
        "physical_gates": [
            {"id": gate, "requires": list(req), "status": "NOT_VERIFIED", "evidence_source_required": "physical_windows"}
            for gate, req in PHYSICAL_GATES
        ],
...
def campaign_progress_v4(evidence: dict[str, bool]) -> dict[str, Any]:
    verified: list[str] = []
    next_gate = None
    for gate, req in PHYSICAL_GATES:
        if all(r in verified for r in req) and bool(evidence.get(gate, False)):
            ...
            continue
        if all(r in verified for r in req):
            next_gate = gate
        break
    return {
        "verified": verified,
