...
def write_failure(path: Path, stage: str, exc: BaseException) -> int:
    ...
    (path / "LIVE_CONVERSATION_V18.json").write_text(json.dumps(detail, indent=2, ensure_ascii=False), encoding="utf-8")
    summary = [
        "CEO de IAs - Live Conversation Validation v18",
        "Live conversation: FAIL/PARTIAL",
        f"Stage: {stage}",
        f"Error: {type(exc).__name__}: {exc}",
        "Conversation Controller maturity: NOT PROMOTED",
        "Release channel: unchanged",
        "See LIVE_CONVERSATION_V18.json and V18_console.log for details.",
    ]
    (path / "VALIDATION_SUMMARY_V18.txt").write_text("\n".join(summary), encoding="utf-8")
    print("\n".join(summary))
    return 1
...
async def live_run(path: Path) -> int:
    ...
    if os.getenv("CEO_BROWSER_START_URL"):
        ...
        (path / "V18_browser_detected.txt").write_text(browser or "NOT FOUND", encoding="utf-8")

    print("[V18] Building real providers...")
    ...
    print("[V18] Providers: " + ", ".join(f"{p.name}/{getattr(p.kind, 'value', p.kind)}" for p in real))
    ...
    db = path / "live_conversation_v18.db"
    if db.exists():
        try:
            db.unlink()
        except OSError:
    ...
    try:
        while not state.completed_at and time.monotonic() < deadline:
            if task.conversation_turns != last_turns:
                ...
                print(f"[V18] turns={last_turns} status={task.status.value} provider={task.provider_name}")
    ...
    evidence = ValidationEvidence(evidence_id="windows-v18-live-conversation", kind="live", environment="windows-live-provider", passed=passed, detail=json.dumps(report, ensure_ascii=False, default=str))
    registry.record("conversation_controller", evidence)
    registry.record("live_ai_provider_validation", ValidationEvidence(evidence_id="windows-v18-live-provider", kind="live", environment="windows-live-provider", passed=passed, detail=json.dumps(report, ensure_ascii=False, default=str)))
    registry.record("scheduler", ValidationEvidence(evidence_id="windows-v18-live-scheduler", kind="live", environment="windows-live-provider", passed=passed, detail=json.dumps(report, ensure_ascii=False, default=str)))
    if task.provider_name and "browser" in task.provider_name.lower():
        registry.record("browser_worker", ValidationEvidence(evidence_id="windows-v18-live-browser-chat", kind="live", environment="windows-live-provider", passed=passed, detail=json.dumps(report, ensure_ascii=False, default=str)))
    ...
    (path / "LIVE_CONVERSATION_V18.json").write_text(json.dumps(report, indent=2, ensure_ascii=False, default=str), encoding="utf-8")
    summary = [
        "CEO de IAs - Live Conversation Validation v18",
...
async def main() -> int:
    path = outdir()
    (path / "V18_PYTHON_STARTED.txt").write_text(time.strftime("%Y-%m-%d %H:%M:%S"), encoding="utf-8")
    try:
