...
def emit(path: Path, message: str) -> None:
    print(message, flush=True)
    try:
        with (path / "V20_console.log").open("a", encoding="utf-8") as fh:
            fh.write(message + "\n")
            fh.flush()
    except OSError:
        pass
...
def write_progress(path: Path, **payload) -> None:
    try:
        (path / "V20_PROGRESS.json").write_text(
...
def write_failure(path: Path, stage: str, exc: BaseException) -> int:
    ...
    (path / "LIVE_CONVERSATION_V20.json").write_text(json.dumps(detail, indent=2, ensure_ascii=False), encoding="utf-8")
    summary = [
        "CEO de IAs - Live Conversation Validation v20",
        "Live conversation: FAIL/PARTIAL",
        f"Stage: {stage}",
        f"Error: {type(exc).__name__}: {exc}",
        "Conversation Controller maturity: NOT PROMOTED",
        "Release channel: unchanged",
        "See LIVE_CONVERSATION_V20.json and V20_console.log for details.",
    ]
    (path / "VALIDATION_SUMMARY_V20.txt").write_text("\n".join(summary), encoding="utf-8")
    emit(path, "\n".join(summary))
    return 1
...
async def live_run(path: Path) -> int:
    ...
    if os.getenv("CEO_BROWSER_START_URL"):
        ...
        (path / "V20_browser_detected.txt").write_text(browser or "NOT FOUND", encoding="utf-8")

    emit(path, "[V20] Building real providers...")
    ...
    emit(path, "[V20] Providers: " + ", ".join(f"{p.name}/{getattr(p.kind, 'value', p.kind)}" for p in real))
    # Bound each browser turn so a UI change cannot make the validation appear frozen.
    for provider in real:
        transport = getattr(provider, "transport", None)
        config = getattr(transport, "config", None)
        if config is not None and hasattr(config, "response_timeout_ms"):
            config.response_timeout_ms = min(int(config.response_timeout_ms), 90_000)
            emit(path, f"[V20] Browser turn timeout: {config.response_timeout_ms // 1000}s")
    ...
    db = path / "live_conversation_v20.db"
    if db.exists():
        try:
            db.unlink()
        except OSError:
    ...
    emit(path, "[V20] Starting scheduler...")
    sched.start()
    # Hard wall-clock bound: this diagnostic must never sit silently for many minutes.
    ...
    try:
        while not state.completed_at and time.monotonic() < deadline:
            ...
            if task.conversation_turns != last_turns:
                ...
                emit(path, f"[V20] turns={last_turns} status={task.status.value} provider={task.provider_name}")
            if now - last_heartbeat >= 5.0:
                elapsed = round(time.perf_counter() - started, 1)
                remaining = max(0, int(deadline - now))
                err = task.metadata.get("last_provider_error")
                emit(path, f"[V20] heartbeat elapsed={elapsed}s remaining<={remaining}s status={task.status.value} turns={task.conversation_turns} last_error={err or 'NONE'}")
                write_progress(
                    path, elapsed_seconds=elapsed, remaining_seconds=remaining,
                    status=task.status.value, turns=task.conversation_turns,
                    provider=task.provider_name, conversation_id_present=bool(task.conversation_id),
        ...
        emit(path, "[V20] Stopping scheduler (bounded to 15s)...")
        try:
            await asyncio.wait_for(sched.stop(), timeout=15)
        except asyncio.TimeoutError:
            stop_timeout = True
            emit(path, "[V20] WARN: scheduler.stop exceeded 15s; continuing to write diagnostics.")
        except Exception as exc:
            emit(path, f"[V20] WARN: scheduler.stop error: {type(exc).__name__}: {exc}")
    ...
    report = {
        "passed": passed,
        "browser_lifecycle_fix": "v20 observable bounded run + 90s browser turn cap + bounded scheduler stop",
        "timed_out": timed_out,
        "scheduler_stop_timed_out": stop_timeout,
        ...
        "last_provider_error": task.metadata.get("last_provider_error"),
        "provider_errors": list(task.metadata.get("provider_errors", [])),
    }

    registry = registry_for(path)
    evidence = ValidationEvidence(evidence_id="windows-v20-live-conversation", kind="live", environment="windows-live-provider", passed=passed, detail=json.dumps(report, ensure_ascii=False, default=str))
    registry.record("conversation_controller", evidence)
    registry.record("live_ai_provider_validation", ValidationEvidence(evidence_id="windows-v20-live-provider", kind="live", environment="windows-live-provider", passed=passed, detail=json.dumps(report, ensure_ascii=False, default=str)))
    registry.record("scheduler", ValidationEvidence(evidence_id="windows-v20-live-scheduler", kind="live", environment="windows-live-provider", passed=passed, detail=json.dumps(report, ensure_ascii=False, default=str)))
    if task.provider_name and "browser" in task.provider_name.lower():
        registry.record("browser_worker", ValidationEvidence(evidence_id="windows-v20-live-browser-chat", kind="live", environment="windows-live-provider", passed=passed, detail=json.dumps(report, ensure_ascii=False, default=str)))
    ...
    (path / "LIVE_CONVERSATION_V20.json").write_text(json.dumps(report, indent=2, ensure_ascii=False, default=str), encoding="utf-8")
    summary = [
        "CEO de IAs - Live Conversation Validation v20",
        ...
        f"Last provider error: {report.get('last_provider_error') or 'NONE'}",
...
async def main() -> int:
    path = outdir()
    (path / "V20_PYTHON_STARTED.txt").write_text(time.strftime("%Y-%m-%d %H:%M:%S"), encoding="utf-8")
    try:
        (path / "V20_console.log").write_text("", encoding="utf-8")
    except OSError:
        pass
    emit(path, "[V20] Python runtime started; live heartbeat enabled.")
    try:
