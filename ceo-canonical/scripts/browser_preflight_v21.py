...
async def main() -> int:
        ...
        async with async_playwright() as p:
            ...
            try:
                ...
                if report["composer_visible"]:
                    (out / "composer_selector_v21.txt").write_text(report["selector"], encoding="utf-8")
                else:
                    # Preserve enough diagnostics to fix the next issue without guessing.
                    try:
                        await page.screenshot(path=str(out / "BROWSER_PREFLIGHT_V21.png"), full_page=True)
                    except Exception:
                        pass
                    try:
                        html = await page.content()
                        (out / "BROWSER_PREFLIGHT_V21.html").write_text(html, encoding="utf-8")
                    except Exception:
                        pass
            finally:
                await ctx.close()
    ...
    (out / "BROWSER_PREFLIGHT_V21.json").write_text(json.dumps(report, indent=2, ensure_ascii=False), encoding="utf-8")
    if report["composer_visible"]:
    ...
    print("Se guardaron BROWSER_PREFLIGHT_V21.png/html para diagnostico.")
    return 1
