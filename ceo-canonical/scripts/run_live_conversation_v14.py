...
async def main() -> int:
    ...
    # v14: resolve the Windows browser path inside Python as a fallback, avoiding
    # fragile cmd.exe START/quoting behavior with paths containing spaces.
    if os.getenv("CEO_BROWSER_START_URL") and not os.getenv("CEO_CHROMIUM_EXECUTABLE"):
        ...
        detected = next((p for p in candidates if p.is_file()), None)
        if detected is not None:
            os.environ["CEO_CHROMIUM_EXECUTABLE"] = str(detected)
    ...
    if not real:
        report = {
            "passed": False,
            "error": "No real provider configured. Use OPENAI_API_KEY/GEMINI_API_KEY or the browser-login path in VALIDAR_CONVERSACION_REAL_V14.cmd.",
        }
        (outdir / "LIVE_CONVERSATION_V14.json").write_text(json.dumps(report, indent=2), encoding="utf-8")
    ...
    db = outdir / "live_conversation_v14.db"
    ...
    evidence = ValidationEvidence(
        evidence_id="windows-v14-live-conversation",
        kind="live",
    ...
    registry.record("live_ai_provider_validation", ValidationEvidence(
        evidence_id="windows-v14-live-provider",
        kind="live",
        environment="windows-live-provider",
        passed=passed,
        detail=json.dumps(report, ensure_ascii=False, default=str),
    ...
    registry.record("scheduler", ValidationEvidence(
        evidence_id="windows-v14-live-scheduler",
        kind="live",
        environment="windows-live-provider",
        passed=passed,
        detail=json.dumps(report, ensure_ascii=False, default=str),
    ...
    if task.provider_name and "browser" in task.provider_name.lower():
        registry.record("browser_worker", ValidationEvidence(
            evidence_id="windows-v14-live-browser-chat",
            kind="live",
            environment="windows-live-provider",
            passed=passed,
            detail=json.dumps(report, ensure_ascii=False, default=str),
    ...
    (outdir / "LIVE_CONVERSATION_V14.json").write_text(
        json.dumps(report, indent=2, ensure_ascii=False, default=str), encoding="utf-8"
    )
    summary = [
        "CEO de IAs - Live Conversation Validation v14",
    ...
    (outdir / "VALIDATION_SUMMARY_V14.txt").write_text("\n".join(summary), encoding="utf-8")
    print("\n".join(summary))
    print(f"\nDetailed report: {outdir / 'LIVE_CONVERSATION_V14.json'}")
