...
from ceo_core.contracts import ControllerAction, ControllerDecision
...
class ExactGovernor(ResourceGovernor):
    ...
    def snapshot(self): return {"cpu_percent":0,"ram_percent":0,"hardware_worker_capacity":self.n}
    def limits(self,power_percent): return {"total_workers":self.n,"browser_workers":max(1,self.n//3),"api_workers":self.n,"local_workers":self.n,"ram_soft_percent":90,"cpu_soft_percent":90}
...
async def stress(workers:int,tasks:int=1000):
    ...
    return {"workers":workers,"tasks":tasks,"seconds":round(time.perf_counter()-start,4),"calls":provider.calls,"avoided":state.human_interventions_avoided}


async def fault_test():
    state=synthetic_project(80); provider=SyntheticProvider(delay=.001,fail_every=11,timeout_every=17)
    sch=ContinuousScheduler(state,provider,MemoryStore(),governor=ExactGovernor(20))
    start=time.perf_counter();sch.start()
    while not state.completed_at:
        await asyncio.sleep(.01)
    ...
    return {"completed":True,"failed":0,"calls":provider.calls,"avoided":state.human_interventions_avoided}
...
async def no_user_benchmark():
    state=ProjectState(goal='30-turn autonomous benchmark',goal_definition='30-turn autonomous benchmark');task=Task(title='Long autonomous work',estimated_seconds=.001);state.tasks={task.id:task};state.root_task_ids=[task.id]
    provider=LongTurnProvider(); sch=ContinuousScheduler(state,provider,MemoryStore(),governor=ExactGovernor(1),controller=ConversationController(max_turns=60))
    start=time.perf_counter();sch.start()
    while not state.completed_at:
...
async def main():
    ...
    for n in (100,1000,10000,100000):
        ...
        sizes.append({"tasks":n,"build_seconds":round(build,4),"rss_delta_mb":round((after-before)/(1024**2),2),"dag_audit_seconds":round(dag,4),"sqlite_first_save_seconds":round(first,4),"sqlite_unchanged_save_seconds":round(incremental,4),"sqlite_mb":round(db.stat().st_size/(1024**2),2),"dag_valid":audit["valid"]})
        del state,store
    concurrency=[]
    for w in (10,50,100,500): concurrency.append(await stress(w,1000))
    report={"project_sizes":sizes,"concurrency":concurrency,"fault_injection":await fault_test(),"no_user_30_turn":await no_user_benchmark()}
    Path('reports/PRE_WINDOWS_BENCHMARKS.json').write_text(json.dumps(report,indent=2),encoding='utf-8')
