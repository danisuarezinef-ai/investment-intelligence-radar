...
def audit(root='.'):
    ...
    def compat_text(r,path):
        # The legacy audit's START_NOT_V10 check is preserved against a virtual legacy
        # entrypoint. Actual production entrypoint is checked strictly below as v22.
    ...
    for n in range(11,23):
        ...
        expected=f'import cloud_service_v{n-1} as base{n-1}'
        if expected not in text:findings.append(f'V{n}_CHAIN_IMPORT_INVALID:{expected}')
        compact=text.replace(' ','')
        if 'REAL_TRADING=False' not in compact:findings.append(f'V{n}_TRADING_BOUNDARY_MISSING')
    ...
    for token in ('automatic_promotion\':False','automatic_release\':False','setup_1_6_allowed\':False','live_execution_allowed\':False','REAL_TRADING=False'):
        if token not in cert.replace(' ', ''):findings.append('CERTIFICATION_SAFETY_TOKEN_MISSING:'+token)
