...
$('provider').textContent=(live?'IA conectada':(keyRecognized?'Clave Gemini reconocida · IA bloqueada':'IA no activa'))+(schedulerAlive?' · motor activo':' · motor detenido');
...
class CEOEngine:
    ...
    async def activate_gemini(self, raw_key: str):
        # Explicit replacement path: a pasted key is always tested independently
        # from any previously stored DPAPI credential. The old secret is only
        # replaced after the new key is authenticated by Gemini.
        ...
        if not key:
            self.gemini_key_recognized = False
            self.gemini_key_status = "INVALID_FORMAT"
        ...
        if not probe.get("ok"):
            ...
            # If listModels succeeded, Google authenticated the key. A 429 or
            # model availability problem must not be presented as "key invalid".
            recognized = models_visible > 0 and status not in {"401", "UNAUTHENTICATED", "INVALID_ARGUMENT"}
            self.gemini_key_recognized = bool(recognized)
            self.gemini_key_status = status
            self.gemini_validation_error = f"{status}: {detail}"[:900]
            self.execution_enabled = False
            self.gemini_model = None
            if recognized:
                # Explicit user replacement: persist an authenticated key even if
                # generation is temporarily quota/model blocked, so Retry can use
                # this NEW key later without falling back to the old credential.
                saved = _save_windows_dpapi_gemini_key(key)
                self.gemini_key = key
                self.provider_mode = "gemini-key-recognized-blocked"
                ...
                return await self.snapshot()
            self.provider_mode = "gemini-validation-failed"
            raise RuntimeError(f"Gemini rechazó o no pudo autenticar la clave: {detail}")
    ...
    async def revalidate_stored_gemini(self):
        ...
        try:
            return await self.activate_gemini(key)
        except Exception as exc:
            self.gemini_validation_error = f"{type(exc).__name__}: {exc}"[:900]
            self.provider_mode = "gemini-validation-failed"
            raise
