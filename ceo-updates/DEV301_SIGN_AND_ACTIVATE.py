...
PACKAGE_NAME = "CEO_1.5.77-rc1-reliability-control-plane.zip"
PACKAGE_SIZE = 1136201
PACKAGE_SHA256 = "4c8058c74169cb3da71e33004c8efe0e1570749c88a015e812aaed30727ea433"
RAW_BASE = f"https://raw.githubusercontent.com/{REPO}/{BRANCH}/{PREFIX}"
DRAFT_URL = RAW_BASE + "DEV301_MANIFEST_UNSIGNED.json"
...
def outbox_file() -> Path:
    p = user_data_root() / "updates" / "outbox" / "DEV301_MANIFEST_SIGNED.json"
    p.parent.mkdir(parents=True, exist_ok=True)
    return p


def status_file() -> Path:
    p = user_data_root() / "updates" / "bridge" / "DEV301_BRIDGE_STATUS.json"
    p.parent.mkdir(parents=True, exist_ok=True)
    return p
...
def fetch_bytes(url: str, timeout: int = 120) -> bytes:
    ...
    req = urllib.request.Request(
        ...
        headers={
            ...
            "Pragma": "no-cache",
            "User-Agent": "CEO-de-IAs-DEV301-major-release-bridge",
        },
    )
...
def verify_package_contract(package: Path) -> dict:
    ...
    with zipfile.ZipFile(package, "r") as zf:
        ...
        if len(names) != len(required) or names != set(required):
            raise RuntimeError("PACKAGE_CONTAINS_UNDECLARED_PATHS")
...
def attempt_publish(signed: dict, verifier) -> tuple[bool, str]:
    ...
    with tempfile.TemporaryDirectory(prefix="ceo-dev301-publish-") as td:
        repo = Path(td) / "repo"
        clone = run_git([
        ...
        if clone.returncode != 0:
            return False, "GIT_CLONE_FAILED:" + (clone.stderr or clone.stdout)[-1000:]

        updates = repo / "ceo-updates"
        staged_package = updates / PACKAGE_NAME
        if not staged_package.is_file():
            return False, "REMOTE_STAGED_PACKAGE_MISSING"
        if staged_package.stat().st_size != PACKAGE_SIZE or sha256_file(staged_package) != PACKAGE_SHA256:
        ...
        signed_path = updates / "DEV301_MANIFEST_SIGNED.json"
        signed_path.write_text(signed_text, encoding="utf-8")
        ...
        add = run_git(["add", "ceo-updates/manifest.json", "ceo-updates/DEV301_MANIFEST_SIGNED.json"], cwd=repo)
        if add.returncode != 0:
            return False, "GIT_STAGE_FAILED:" + (add.stderr or add.stdout)[-1000:]

        commit = run_git(["commit", "-m", "Activate signed DEV301 major reliability update"], cwd=repo)
        if commit.returncode not in (0, 1):
            return False, "GIT_COMMIT_FAILED:" + (commit.stderr or commit.stdout)[-1000:]

        push = run_git(["push", "origin", BRANCH], cwd=repo, timeout=180)
    ...
    try:
        ...
        if len(remote_pkg) != PACKAGE_SIZE:
            return False, "REMOTE_PACKAGE_SIZE_FAILED"
        if hashlib.sha256(remote_pkg).hexdigest() != PACKAGE_SHA256:
            return False, "REMOTE_PACKAGE_SHA_FAILED"
    except Exception as exc:
...
def main() -> int:
    ...
    try:
        ...
        exact = {
            ...
            "url": PACKAGE_URL,
            "min_app_version": "1.5.58-rc1-productive-stall-escape",
        }
        for key, expected in exact.items():
            if draft.get(key) != expected:
                return fail("PREPARED_MANIFEST_MISMATCH", f"{key}: {draft.get(key)!r} != {expected!r}", 7)

        with tempfile.TemporaryDirectory(prefix="ceo-dev301-sign-") as td:
            ...
            if not signer.configured or not signer.self_test:
                return fail(
                    "SIGNER_UNAVAILABLE",
                    "La autoridad persistente no está configurada o no supera self-test. No se ha creado ni rotado ninguna clave.",
                    8,
                )
            ...
            payload = {
                "ok": True,
                "status": "DEV301_PUBLISHED" if published else "DEV301_SIGNED_NEEDS_UPLOAD",
                "version": VERSION,
                "release_sequence": RELEASE_SEQUENCE,
                "release_id": RELEASE_ID,
        ...
        return fail("DEV301_BRIDGE_FAILED", f"{type(exc).__name__}: {exc}", 30)
