...
$Host.UI.RawUI.WindowTitle = 'CEO de IAs - DEV313 Native Transport Integrity 1.5.89'

$expected = '1.5.86-rc1-provider-session-integrity'
$target = '1.5.89-rc1-native-transport-integrity'
$manifestUrl = 'https://raw.githubusercontent.com/danisuarezinef-ai/investment-intelligence-radar/ceo-update-channel/ceo-updates/DEV313_MANIFEST_UNSIGNED.json'
$logDir = Join-Path $env:LOCALAPPDATA 'CEO de IAs\diagnostics'
$logFile = Join-Path $logDir 'DEV313_NATIVE_TRANSPORT_INTEGRITY_ACTIVACION.log'
...
function Finish([int]$code) {
    ...
    Write-Host '============================================================'
    if ($code -eq 0) {
        Write-Host '  DEV313 / 1.5.89 PUBLICADA POR EL PROPIO CEO'
    } else {
        Write-Host '  DEV313 BLOQUEADA DE FORMA SEGURA'
...
try {
    ...
    Log 'Solicitando verificacion, firma y publicacion interna de DEV313.'
    try {
    ...
    Log 'DEV313 publicada y verificada.'
    Write-Host ''
    if ($visible) {
        Write-Host 'CEO ya ve 1.5.89 en el canal.'
        Write-Host 'En CEO pulsa Comprobar ahora y despues Instalar y reiniciar -> Confirmar.'
    } else {
        Write-Host '1.5.89 esta publicada. Pulsa Comprobar ahora dentro de CEO.'
    }
    Write-Host ''
    Write-Host 'NO generes otra clave Gemini.'
    Write-Host 'DEV313 fuerza IPv4 en httpx y usa HTTPS nativo de Windows como fallback.'
    Write-Host 'Tambien incluye DEV311 y DEV312.'
    Write-Host ''
    Finish 0
}
catch {
