...
$Host.UI.RawUI.WindowTitle = 'CEO de IAs - DEV308 Executable Route Integrity 1.5.84'

$expected = '1.5.83-rc1-autonomy-runtime-integrity'
$target = '1.5.84-rc1-executable-route-integrity'
$manifestUrl = 'https://raw.githubusercontent.com/danisuarezinef-ai/investment-intelligence-radar/ceo-update-channel/ceo-updates/DEV308_MANIFEST_UNSIGNED.json'
$logDir = Join-Path $env:LOCALAPPDATA 'CEO de IAs\diagnostics'
$logFile = Join-Path $logDir 'DEV308_EXECUTABLE_ROUTE_INTEGRITY_ACTIVACION.log'
...
function Finish([int]$code) {
    ...
    Write-Host '============================================================'
    if ($code -eq 0) {
        Write-Host '  DEV308 / 1.5.84 PUBLICADA POR EL PROPIO CEO'
    } else {
        Write-Host '  DEV308 BLOQUEADA DE FORMA SEGURA'
        Write-Host ('  Codigo: ' + $code)
        Write-Host '  La instalacion 1.5.83 sigue intacta.'
    }
    Write-Host '============================================================'
...
try {
    ...
    if ($null -eq $port) {
        Log 'ERROR: no se encontro CEO 1.5.83 activo.'
        Finish 6
    }

    Log ('CEO 1.5.83 localizado en puerto '+$port)
    $body=@{manifest_url=$manifestUrl}|ConvertTo-Json -Compress
    $uri='http://127.0.0.1:'+$port+'/api/internal-release/activate-prepared'
    Log 'Solicitando verificacion, firma y publicacion interna de DEV308.'
    ...
    if (-not $r.ok) {
        Log ('ERROR: ok=false status='+[string]$r.status+' detail='+[string]$r.detail)
        Finish 11
    }
    if ([string]$r.status -ne 'PUBLISHED') {
        Log ('ERROR: status='+[string]$r.status)
        Finish 12
    }
    ...
    Log 'DEV308 publicada y verificada.'
    Write-Host ''
    if ($visible) {
        Write-Host 'CEO ya ve 1.5.84 en el canal.'
        Write-Host 'En CEO pulsa Comprobar ahora y despues Actualizar / Instalar y reiniciar -> Confirmar.'
    } else {
        Write-Host '1.5.84 esta publicada, pero esta instancia aun no la refleja.'
        Write-Host 'Cierra y abre CEO una vez; despues pulsa Comprobar ahora e instala 1.5.84.'
    }
    Write-Host ''
    Write-Host 'Tras reiniciar, NO crees un objetivo nuevo: deja que 1.5.84 recupere el ABG actual al 73.3%.'
    Write-Host ''
