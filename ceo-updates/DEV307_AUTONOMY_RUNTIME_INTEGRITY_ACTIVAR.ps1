...
$Host.UI.RawUI.WindowTitle = 'CEO de IAs - DEV307 Autonomy Runtime Integrity 1.5.83'

$expected = '1.5.82-rc1-autonomy-bootstrap'
$target = '1.5.83-rc1-autonomy-runtime-integrity'
$manifestUrl = 'https://raw.githubusercontent.com/danisuarezinef-ai/investment-intelligence-radar/ceo-update-channel/ceo-updates/DEV307_MANIFEST_UNSIGNED.json'
$logDir = Join-Path $env:LOCALAPPDATA 'CEO de IAs\diagnostics'
$logFile = Join-Path $logDir 'DEV307_AUTONOMY_RUNTIME_INTEGRITY_ACTIVACION.log'
...
function Finish([int]$code) {
    ...
    Write-Host '============================================================'
    if ($code -eq 0) {
        Write-Host '  DEV307 / 1.5.83 PUBLICADA POR EL PROPIO CEO'
    } else {
        Write-Host '  DEV307 BLOQUEADA DE FORMA SEGURA'
        Write-Host ('  Codigo: ' + $code)
        Write-Host '  La instalacion 1.5.82 sigue intacta.'
    }
    Write-Host '============================================================'
...
try {
    ...
    if ($null -eq $port) {
        Log 'ERROR: no se encontro CEO 1.5.82 activo.'
        Finish 6
    }

    Log ('CEO 1.5.82 localizado en puerto '+$port)
    $body=@{manifest_url=$manifestUrl}|ConvertTo-Json -Compress
    $uri='http://127.0.0.1:'+$port+'/api/internal-release/activate-prepared'
    Log 'Solicitando verificacion, firma y publicacion interna de DEV307.'
    try {
    ...
    # Ask the running backend to see the published release. Propagation is bounded;
    # publication success does not become failure merely because the remote view is delayed.
    $visible=$false
    ...
    Log 'DEV307 publicada y verificada.'
    Write-Host ''
    if ($visible) {
        Write-Host 'CEO ya ve 1.5.83 en el canal.'
        Write-Host 'En CEO pulsa Comprobar ahora y despues Actualizar / Instalar y reiniciar -> Confirmar.'
    } else {
        Write-Host '1.5.83 esta publicada, pero esta instancia aun no la refleja.'
        Write-Host 'Cierra y abre CEO una vez; despues pulsa Comprobar ahora e instala 1.5.83.'
    }
    Write-Host ''
