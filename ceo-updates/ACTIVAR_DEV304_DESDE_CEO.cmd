...
echo ============================================================
echo   CEO de IAs - activar Provider Resilience 1.5.80
echo ============================================================
echo.
echo Este archivo NO firma por fuera, NO instala y NO modifica claves.
echo Pide al CEO 1.5.79 que use su PreparedReleaseBridge interno:
echo   verificar -> firmar -> publicar DEV304.
...
powershell.exe -NoProfile -ExecutionPolicy Bypass -Command ^
  ...
  "if($null -eq $port){Write-Host '[BLOQUEADO] No se encontro una instancia sana de CEO 1.5.79.';exit 6};" ^
  "Write-Host ('[OK] CEO 1.5.79 localizado en puerto '+$port);" ^
  "$body=@{manifest_url='%MANIFEST_URL%'}|ConvertTo-Json -Compress;" ^
  "Write-Host '[1/3] CEO descarga y verifica DEV304...';" ^
  "Write-Host '[2/3] CEO firma y publica 1.5.80 con su autoridad local...';" ^
  ...
  "Write-Host '[3/3] DEV304 publicada y verificada por CEO.';" ^
...
echo ============================================================
echo Codigo: %RC%
echo CEO 1.5.79 sigue activo y no se ha instalado nada.
pause
exit /b %RC%

:OK
echo ============================================================
echo   1.5.80 FIRMADA Y PUBLICADA POR EL PROPIO CEO
echo ============================================================
echo.
echo En CEO pulsa Comprobar ahora.
echo Debe aparecer 1.5.80 disponible.
