...
$Host.UI.RawUI.WindowTitle = 'CEO de IAs - DEV310 Provider Session Integrity 1.5.86'

$expected = '1.5.85-rc1-closure-convergence'
$target = '1.5.86-rc1-provider-session-integrity'
$manifestUrl = 'https://raw.githubusercontent.com/danisuarezinef-ai/investment-intelligence-radar/ceo-update-channel/ceo-updates/DEV310_MANIFEST_UNSIGNED.json'
$logDir = Join-Path $env:LOCALAPPDATA 'CEO de IAs\diagnostics'
$logFile = Join-Path $logDir 'DEV310_PROVIDER_SESSION_INTEGRITY_ACTIVACION.log'
...
function Finish([int]$code) {
    ...
    Write-Host '============================================================'
    if ($code -eq 0) {
        Write-Host '  DEV310 / 1.5.86 PUBLICADA POR EL PROPIO CEO'
    } else {
        Write-Host '  DEV310 BLOQUEADA DE FORMA SEGURA'
        Write-Host ('  Codigo: ' + $code)
        Write-Host '  La instalacion 1.5.85 sigue intacta.'
    }
    Write-Host '============================================================'
...
try {
    ...
    if ($null -eq $port) {
        Log 'ERROR: no se encontro CEO 1.5.85 activo.'
        Finish 6
    }

    Log ('CEO 1.5.85 localizado en puerto '+$port)
    $body=@{manifest_url=$manifestUrl}|ConvertTo-Json -Compress
    $uri='http://127.0.0.1:'+$port+'/api/internal-release/activate-prepared'
    Log 'Solicitando verificacion, firma y publicacion interna de DEV310.'
    try {
    ...
    Log 'DEV310 publicada y verificada.'
    Write-Host ''
    if ($visible) {
        Write-Host 'CEO ya ve 1.5.86 en el canal.'
        Write-Host 'En CEO pulsa Comprobar ahora y despues Actualizar / Instalar y reiniciar -> Confirmar.'
    } else {
        Write-Host '1.5.86 esta publicada, pero esta instancia aun no la refleja.'
        Write-Host 'Cierra y abre CEO una vez; despues pulsa Comprobar ahora e instala 1.5.86.'
    }
    Write-Host ''
    Write-Host 'IMPORTANTE: no generes otra clave Gemini.'
    Write-Host 'Tras reiniciar, CEO reutilizara la clave cifrada guardada y distinguira'
    Write-Host 'entre clave rechazada y proveedor/cuota temporalmente no disponible.'
    Write-Host 'No crees otro objetivo: deja continuar el objetivo actual.'
    Write-Host ''
