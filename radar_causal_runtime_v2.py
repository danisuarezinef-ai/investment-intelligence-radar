"""Throttled causal graph refresh reconciled from legacy PR20.

Causal refresh is observational only; it cannot bypass abstention, risk, promotion,
or real-trading gates.
"""
...
def refresh_causal_if_due(interval_seconds=900,hours=168,force=False):
    ...
    if not force and t-last<float(interval_seconds):
        c.close();return {'refreshed':False,'due_in_seconds':max(0,int(interval_seconds-(t-last))),'summary':graph_summary(),'promotion_authorized':False,'real_trading':False}
    ...
    return {'refreshed':True,'edges_created':created,'summary':graph_summary(),'promotion_authorized':False,'real_trading':False}
