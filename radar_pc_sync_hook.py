...
def _post(token,payload,timeout=35):
    ...
    req=urllib.request.Request(CLOUD_BASE+'/pc-sync',data=data,method='POST',headers={'Authorization':'Bearer '+token,'Content-Type':'application/json','User-Agent':'InvestmentIntelligenceRadarDesktopSync/'+PC_SYNC_VERSION})
    with urllib.request.urlopen(req,timeout=timeout) as r:return json.loads(r.read().decode('utf-8'))
...
def _sync_once():
    ...
    settings=_read_json(settings_path);token=str(settings.get('cloud_control_token') or '').strip();node_id=str(settings.get('node_id') or '').strip()
    cloud_cache=_pull_cloud_cache()
    ...
    init_db();c=con()
    try:
        market=c.execute('select id,ts,symbol,price,volume,source from market_snapshots where id>? order by id limit 300',(market_after,)).fetchall()
        events=c.execute('select id,ts,source,title,url,category from information_events where id>? order by id limit 300',(event_after,)).fetchall()
        runs=c.execute('select id,ts,job,status,detail from system_runs where id>? order by id limit 300',(run_after,)).fetchall()
    finally:c.close()
    payload={'node_id':node_id,'name':os.environ.get('COMPUTERNAME','Windows PC'),'app_version':PC_SYNC_VERSION,'capabilities':['local-collector','paper-simulator','desktop-ui','persistent-sync','cloud-cache'],
    ...
    state['last_success']=time.time();state['last_error']='';state['last_counts']={'market':len(market),'events':len(events),'runs':len(runs)};state['cloud_cache_fetched_at']=cloud_cache.get('fetched_at')
    pending={'market':max(0,len(market)-300),'events':max(0,len(events)-300),'runs':max(0,len(runs)-300)};state['pending_hint']=pending
    _write_json_atomic(state_path,state);return {'enabled':True,**state['last_counts']}
...
def _loop():
    ...
    while True:
        try:_sync_once()
            ...
            state_path=os.path.join(_data_dir(),'pc_sync_state.json');state=_read_json(state_path);state['last_error']='HTTP '+str(exc.code);state['last_attempt']=time.time();_write_json_atomic(state_path,state)
        except Exception as exc:
            state_path=os.path.join(_data_dir(),'pc_sync_state.json');state=_read_json(state_path);state['last_error']=str(exc)[:500];state['last_attempt']=time.time();_write_json_atomic(state_path,state)
        time.sleep(60)

if os.name=='nt':threading.Thread(target=_loop,name='radar-pc-bidirectional-sync',daemon=True).start()
