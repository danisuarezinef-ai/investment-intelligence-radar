        ...
        if root is not None and main is not None:
            def build():
                try:
                    ...
                    tk.Label(box,text='Intelligence v2 · aprendizaje y sincronización',bg=panel,fg=text,font=('Segoe UI',11,'bold')).pack(anchor='w')
                    ...
                    def refresh():
                        cache=_read_json(os.path.join(_data_dir(),'cloud_intelligence_cache.json'));sync=_read_json(os.path.join(_data_dir(),'pc_sync_state.json'));learning=cache.get('learning') or {};model=learning.get('model') or {};reg=learning.get('regime') or {};counts=learning.get('counts') or {};last=sync.get('last_success');age=(time.time()-last) if last else 1e9;ok=age<180 and not sync.get('last_error');status.set(('SINCRONIZACIÓN OK · ' if ok else 'REVISAR SINCRONIZACIÓN · ')+f"modelo {model.get('version','—')} · régimen {reg.get('regime','—')}");detail.set(f"Predicciones {counts.get('predictions',0)} · resultados {counts.get('outcomes',0)} · ciclos de aprendizaje {counts.get('learning_cycles',0)} · PC→Cloud {'OK' if ok else sync.get('last_error','pendiente')} · Cloud→PC {'OK' if cache.get('fetched_at') else 'pendiente'} · trading real OFF")
