...
from radar_core import con, init_db, _daily_series
...
def benchmark_agents():
    ...
    bench=_daily_series(BENCHMARK_SYMBOL,365);bench_vals=[x[1] for x in bench];bench_m=_metrics(bench_vals)
    for aid,cfg in AGENTS.items():
        ...
        m.update({'agent_id':aid,'name':cfg['name'],'marks':len(vals),'trades':int(trades[0] or 0),'costs':float(trades[1] or 0),'positions':int(positions or 0),'benchmark':BENCHMARK_SYMBOL,'benchmark_return_pct':bench_m['return_pct'],'excess_return_pct':m['return_pct']-bench_m['return_pct']})
        out.append(m)
