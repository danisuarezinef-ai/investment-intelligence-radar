...
def _verified_market_gate():
    """Forward-only market-data gate backed by the current live Block E probe.

    This deliberately does not use historical fixtures, backfill, or deterministic-only
    evidence. Formal maturity credit is allowed only when the current runtime's Block E
    live provider probe itself is PASS.
    """
    try:
        b=base36.base35.base34.base33.base32.block_e_board() or {}
        live=b.get('live') or {}
        return b.get('status')=='PASS' and live.get('status')=='PASS' and live.get('market_data_test_set')=='READY'
    except Exception:
        return False

def _install_forward_market_gate():
    # v23 owns the formal forward-maturity writer inherited by v37.
    # Replace only its stale task-114 adapter; all other maturity criteria remain unchanged.
    target=base36.base35.base34.base29.base27.base26.base25.base24.base23
    target._market_gate=_verified_market_gate
