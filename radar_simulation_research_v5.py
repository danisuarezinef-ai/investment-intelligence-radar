...
def expanded_stress_suite(config,days=None):
    days=list(days or _series_by_day());cut=max(90,len(days)-60);cut=min(cut,max(1,len(days)-30))
    scenarios={'base':({},{}),'costs_2x':({}, {'cost_multiplier':2}),'costs_4x':({}, {'cost_multiplier':4}),'gap_-12':({'kind':'gap','pct':-12},{}),'gap_-20':({'kind':'gap','pct':-20},{}),'volatility_12':({'kind':'volatility','amp':.12},{}),'volatility_20':({'kind':'volatility','amp':.20},{}),'sideways':({'kind':'sideways'},{}),'missing_data':({'kind':'missing_data','every':5},{}),'shock_-20':({}, {'shock_pct':-20}),'shock_-30':({}, {'shock_pct':-30})}
    rows={}
    for name,(tx,stress) in scenarios.items():
        transformed=_transform_days(days,tx);rows[name]=evaluate_oos(config,transformed[:cut],transformed[cut:],stress)
