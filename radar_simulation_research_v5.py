...
def evaluate_oos(config,history,window,stress=None):
    base=evaluate_config(config,list(history),stress=stress,return_marks=True);full=evaluate_config(config,list(history)+list(window),stress=stress,return_marks=True);out=_partition_metrics(full,base);out['history_days']=len(history);out['oos_days']=len(window);out['evidence_class']='SIMULATED_OOS_ONLY';return out
...
def chronological_walk_forward(config,days=None,folds=4,min_train=90,validation_days=20,test_days=20):
    ...
    for fold in range(max(1,int(folds))):
        ...
        rows.append({'fold':fold+1,'train_days':len(train),'validation_days':len(val),'test_days':len(test),'validation':vr,'test':tr,'train_end':train[-1][0],'validation_end':val[-1][0],'test_end':test[-1][0],'lookahead':False});cursor=t1
