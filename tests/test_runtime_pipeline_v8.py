...
def test_market_telemetry_reports_all_assets_and_observed_fallback(monkeypatch):
    ...
    sample[first]['source']='Stooq Quote'
    monkeypatch.setattr(op,'_latest_market_rows',lambda:sample)
    result=op.market_telemetry()
    assert result['assets_expected']==16
    assert result['assets_observed']==16
    assert result['coverage_complete'] is True
    assert result['failover_activation_verified'] is True
    assert result['provider_attempt_telemetry']=='NOT_IMPLEMENTED'
...
def test_matured_forward_records_preserve_missing_cost_and_benchmark(monkeypatch):
    class C:
        def execute(self,*args):
            return self
        def fetchall(self):
            return [(1,'MSFT','2026-09-09T00:00:00+00:00','2026-09-10T00:00:00+00:00',
                     json.dumps({'decision_state':'WAIT'}),json.dumps({'return':0.01}),
                     '2026-09-10T00:01:00+00:00')]
        def close(self):
            pass
    monkeypatch.setattr(op,'con',lambda:C())
    rows=op.matured_forward_records()
...
def test_operational_pipeline_wires_modules_but_fails_closed(monkeypatch):
    ...
    monkeypatch.setattr(op,'_ranking_rows',lambda:[{'symbol':'MSFT','score':1.0,'risk':'bajo','momentum_score':1.0,
                                                   'liquidity_score':None,'data_quality':1.0,
                                                   'deep_evidence_ready':False,'decision_evidence_complete':False}])
    ...
    assert result['wiring_status']['valuation_engine_v1']=='LIVE_FAIL_CLOSED_MISSING_FUNDAMENTALS'
...
def test_priority_runtime_uses_matured_forward_records(monkeypatch):
    ...
    monkeypatch.setattr(cloud_service_v3,'operational_pipeline',lambda:{
        'forward_records':[{'matured':True,'net_return':None,'excess_return':None}],
        'universe':{'screened':[]},'real_trading':False})
    result=cloud_service_v3.priority_runtime_live()
    assert result['wiring']['priority_forward_records'].startswith('LIVE_MATURED_LEDGER_ONLY')
    assert result['strategy_performance_verified'] is False
    assert result['real_trading'] is False


def test_failover_policy_opens_broken_provider_and_prefers_healthy():
    result=failover_order([
        {'name':'Yahoo query1','calls':4,'failures':4,'consecutive_failures':4},
        {'name':'Stooq.com','calls':10,'failures':0,'consecutive_failures':0},
    ])
