...
def test_autonomous_cycle_runs_paper_and_research(monkeypatch):
    writes=[]
    ...
    monkeypatch.setattr(sim,'run_research_batch',lambda generation:{'status':'COMPLETED','tested':6})
    r=sim.run_autonomous_cycle(force_research=True)
    assert r['status']=='ACTIVE'
    assert r['result']['paper_cycle_status']=='OK'
    assert r['result']['experiments_tested']==6
