...
def test_display_selection_cannot_create_forward_credit():
    snap=observable_snapshot({"completed_cycles":355}, {}, "confidence", "all")
    assert snap["window"]=="all"
    assert snap["metric"]=="confidence"
    assert not any(k in snap for k in ("valid_forward_hours","maturity_credit","promotion","release"))
