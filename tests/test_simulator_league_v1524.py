...
def test_promotion_horizon_is_dynamic_not_fixed_ten_days():
    strong=league.dynamic_required_days(v_gap=45,confidence=.90,equity_gap_pct=8)
    marginal=league.dynamic_required_days(v_gap=5,confidence=.55,equity_gap_pct=.5)
    weak_evidence=league.dynamic_required_days(v_gap=5,confidence=.20,equity_gap_pct=.5)
    assert 2 <= strong < marginal < weak_evidence <= 30
    assert len({strong,marginal,weak_evidence}) == 3


def test_dynamic_promotion_requires_quality_evidence_and_risk_together():
    required=league.dynamic_required_days(20,.80,4)
    before=league.promotion_candidate(92,-4,-3,v_gap=20,confidence=.80,equity_gap_pct=4,common_days=required-1)
    assert before['eligible'] is False and before['evidence_ok'] is False
    good=league.promotion_candidate(92,-4,-3,v_gap=20,confidence=.80,equity_gap_pct=4,common_days=required)
    assert good['eligible'] is True
    assert good['promotion_policy']=='dynamic_v1'
    assert good['promotion_scope']=='SIMULATION_LEAGUE_ONLY'
    assert good['automatic_model_promotion'] is False
    assert good['real_trading'] is False


def test_promotion_risk_guard_blocks_lucky_but_excessive_drawdown():
    required=league.dynamic_required_days(40,.95,12)
    gate=league.promotion_candidate(99,-13,-2,v_gap=40,confidence=.95,equity_gap_pct=12,common_days=required,risk_guard_pp=10)
...
def test_old_champion_becomes_challenger_one_on_dynamic_league_promotion():
    edge=Path('supabase/functions/radar-simulator-league/index.ts').read_text(encoding='utf-8')
    ...
    assert 'dynamic_required_days' in edge
